...
evaluators.class = ast => {
    ...
    const constructor = ast.table.entries.find(x => x.key == "constructor");
    if(constructor) {
        ast.table.entries = ast.table.entries.filter(x => x.key != "constructor");
    }
    ...
    txt += `__class = setmetatable({
        __init = ${(() => {
            ...
            if(constructor.value.type == "function" && typeof constructor.value.selfType === "undefined") {
                constructor.value.selfType = { type: "variable", name: "__base" };
            }
            let t = evaluate(constructor.value);
            method_stack.pop();
            return t;
        })()},
        __base = __base,
        ...
        __index = ${(() => {
            if(!ast.extending) return "__base";
            return `function(self, name)
                local val = rawget(__base, name)
                if val == nil then
                    local super = rawget(self, "__super")
                    if super then return super[name] end
                else return val end
            end`;
        })()},
        __call = function(self, ...)
...
evaluators.super_value = () => "__class.__super";

evaluators.super_call = ast => {
    ast.args.unshift({ type: "variable", name: "self" });
    const method = get_current_method();
    return "__class.__super." + method.name + "(" + ast.args.map(x => evaluate(x)).join(", ") + ")";
};
